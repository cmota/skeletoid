...
class AnalyticsManager internal constructor() : IAnalyticsManager {
    ...
    companion object {
        ...
        private val LOG_TAG = "AnalyticsManager"
    }

    //TODO: List of appenders (it can be improved to an ArrayMap if we want to add the support lib as dependency
    private val analyticsAppenders: MutableMap<String, IAnalyticsAppender>

    init {
        analyticsAppenders = HashMap()
    }

    override fun addAppenders(context: Context, analyticsAppenders: List<IAnalyticsAppender>): Set<String> {
        if (analyticsAppenders.isEmpty()) {
            return HashSet()
        }
    ...
    override fun removeAppenders(context: Context, analyticsIds: Set<String>) {
        if (analyticsAppenders.isEmpty()) {
            return
        }
    ...
    override fun removeAllAppenders() {
        val appendersKeys = ArrayList(analyticsAppenders.keys)
